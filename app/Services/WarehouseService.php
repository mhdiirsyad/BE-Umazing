...
class WarehouseService
{
    ...
    public function attachProducts(int $warehouseId, int $productId, int $stock) {
        $warehouseId = $this->warehouseRepository->getById($warehouseId, ['id']);
        $warehouseId->products()->syncWithoutDetachig([$productId => ['stock' => $stock]]);
    }       

    public function detachProducts(int $warehouseId, int $productId) {
        $warehouseId = $this->warehouseRepository->getById($warehouseId, ['id']);
        $warehouseId->products()->detach($productId);
    }

    Public function UpdateProductStock(int $warehouseId, int $productId, int $stock) {
        $warehouseId = $this->warehouseRepository->getById($warehouseId, ['id']);
        $warehouseId->products()->updateExistingPivot($productId, ['stock' => $stock]);

        return $warehouseId->products()->where('product_id', $productId)->first();
    }
